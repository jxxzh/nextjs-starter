...
## Purpose
基于 Nextjs 的全栈 React 模板工程，提供开箱可用的路由、数据获取、UI 组件与 FSD 分层约定，帮助快速搭建可维护、可扩展的前后端同构应用。
...
- 采用 FSD 分层：shared → entities → features → widgets → app(App Router)，单向依赖仅指向更底层
- 按Slice（业务域）+ Segment（ui/api/model/lib/config）组织代码，避免跨层耦合；底层共享逻辑放在 `src/shared/`
- 路由与页面使用 Nextjs App Router，根据需要选择SSR、CSR、SSG、ISR等模式
- 样式以 Tailwind v4 为主，基础样式位于 `src/shared/styles/`
...
## Domain Context
当前为通用 Nextjs + Shadcn UI 的前端/同构模板项目，尚无特定业务域；新增业务请按 FSD 切片落位并补充对应 specs。
